...
<body>
	...
	$sliderIdToEdit = intval($_GET['sliderId']);
	echo $sliderIdToEdit;

	$slider = R::load('sliders', $sliderIdToEdit);
	...
	if ( count($_FILES) > 0 ) {
		...
		// $slider = R::dispense('sliders');
		$slider->images = $sliderJsonInfo;
		R::store($slider);
		echo "Всё успешно записано в БД.";
	}
	?>

	<form action="edit.php?sliderId=<?=$sliderIdToEdit?>" 
		method="POST" 
